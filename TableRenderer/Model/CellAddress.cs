...
namespace TableRenderer.Model
{
  ...
  public struct CellAddress : IEquatable<CellAddress>
  {
    ...
    /// </summary>
    [DisplayName("Sheet"), Description("The name of the sheet.")]
    public string Sheet
    {
      get { return _sheet; }
      set { _sheet = value; }
    ...
    /// <param name="row">Zero-based row index</param>
    /// <param name="column">Zero-based column index</param>
    public CellAddress(string sheet, int row, int column)
    {
      _sheet = sheet;
      _row = row;
      _column = column;
    ...
    {
      return (this.Sheet ?? "").GetHashCode() ^ this.Row ^ this.Column;
    }
    /// <summary>
    ...
    public override string ToString()
    {
      if (string.IsNullOrEmpty(_sheet))
        return GetColumnDesignator(this.Column) + (this.Row + 1).ToString();
      return string.Format("{0}!{1}{2}", _sheet, GetColumnDesignator(this.Column), this.Row + 1);
    }
